...
export default function TripDetailPage({
  ...
  const loadTrip = useCallback(async () => {
    ...
    setDayPlaceInfo(data.day_place_info ?? {});
    setPlanView(data.plan_view ?? null);
  }, [tripId]);
  ...
  async function handleRemove(tripPlaceId: string) {
    await fetch(`/api/trips/${tripId}/places?place_id=${tripPlaceId}`, {
      method: "DELETE",
    });
    loadTrip();
  }
  ...
  async function handleGeneratePlan() {
    ...
    try {
      ...
      const res = await fetch(`/api/trips/${tripId}/plan`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) {
  ...
  if (!trip) {
    return (
      <div>
        <p className="text-sm text-alert">旅行が見つかりませんでした。</p>
  ...
  return (
    <div className="mt-6 space-y-6">
      ...
      <div className="rounded-lg border border-route/20 bg-card p-4 shadow-sm">
        ...
        <a
          href={`/api/trips/${tripId}/export/kml`}
      ...
      <div className="rounded-lg border border-route/20 bg-card p-4 shadow-sm">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <a
            href={`/api/trips/${tripId}/export/docx`}
          ...
          <a
            href={`/api/trips/${tripId}/export/print`}
            target="_blank"
            rel="noreferrer"
          ...
          <a
            href={`/api/trips/${tripId}/export/ics`}
          ...
          <a
            href={`/api/trips/${tripId}/export/txt`}
